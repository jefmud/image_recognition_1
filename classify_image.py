...
"""Simple image classification with Inception-v3.
...
def main2(_):
  ...
  while rc:
    ...
    if fname is None:
      url = easygui.enterbox(msg='Enter a URL')
      
      if url is None:
        sys.exit(0)
        
      if download_picture(url, dest_filename='image.jpg', overwrite=True):
        fname = 'image.jpg'
    
    if fname:
      data = run_inference_on_image(fname)
      buf = []
      for item in data:
        buf.append('%s (score = %.5f)' % (item['ident'],item['score']))
      rc = easygui.codebox(text='\n'.join(buf))
    rc = easygui.ynbox(msg='Continue?')
